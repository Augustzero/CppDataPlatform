...
bool _xmltodb()
{
    ...
    while (true) 
    {
        ...
        while (true)
        {
            ...
            // 1-入库参数不正确; 3-待入库的表不存在; 4-执行入库的SQL语句失败
...
//从vxmltotable容器中查找的入库参数, 存放在stxmltotable结构体中
...
int _xmltodb(const string &fullfilename,const string &filename)
{
    ...
    // 4) 打开xml文件
    cifile ifile;
    if (ifile.open(fullfilename)==false) {conn.rollback(); return 5;}    // 5-打开文件
